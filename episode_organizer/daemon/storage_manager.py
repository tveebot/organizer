...
class StorageError(Exception):
    """
    Raised to indicate there is a critical error with the storage and the application can not
    proceed. This exception is raised when, for instance, the storage directory is deleted.
    """
    pass
...
class StorageManager:
    ...
    method) the episode file in the correct sub-directory inside the storage directory. The
    directory where the file is stored is determined based on the episode information included
    ...
    def store(self, episode_file: str, episode: Episode):
        """
        This is the entry point of the storage manager. This method is called to store an episode
        file in it correct place, based on its episode information provided in the 'episode'
        argument. If the destination directory does not exist, then it is automatically created.
        The episode file is moved to the destination directory and not copied. Which means that
        after calling this method the file is removed from its current directory.
        ...
        try:
            ...
            raise FileExistsError("File '%s' already exists in '%s'" %
                                  (os.path.basename(episode_file),
                                   os.path.relpath(move_dir, start=self.storage_dir)))

        return os.path.join(move_dir, os.path.basename(episode_file))

    def episode_dir(self, episode: Episode):
        """
        Determines the episode directory based on its information. In its current form,
        the directory of an episode is composed of a season folder of the form 'Season XX',
        where XX indicates the season number to which the episode belongs. This directory is
        included inside a tv show directory with its name. For example, an episode from season 1
        of a tv show called 'Example' is stored in 'Example/Season 01'.
